...
function generatedQuestion(language, category, question, correctAnswer, 
...
function getRandomNumberDifferentToAll(numbers, max) {
  ...
  for (var i = 0; i < noneNumbers.length; ++i) {
    if(result >= noneNumbers[i]) { ++result; }
  }
  return result;
}
...
function addWrongAnswersFromPool(question, answerPool) {
  var numbers = getRandomNumbersDifferentTo(
    answerPool.indexOf(question.correctAnswer), answerPool.length, 3);
...
function generatedQuestionToQuestion(q) {
  var answers = util.shuffle([q.correctAnswer, q.wrongAnswer1, q.wrongAnswer2, 
      q.wrongAnswer3]);

  return storage.createQuestion(q.language, q.category, q.question, answers,
    getIndex(q.correctAnswer, answers));
}
...
function addToDatabase(questions) {
  ...
  for(var i=0; i<questions.length; ++i) {
    storage.storeQuestion(generatedQuestionToQuestion(questions[i]));
  }
}
      ...
      */
      ...
      for (var i = 0; i < row.length; ++i) {
        ...
        if(capital != null && country != null) {
          result.push(new generatedQuestion("en", "Geography", "What is the capital of " + country[1] + "?", capital[1], "1", "2", "3"));
        }
      }
      ...
      */
      ...
      for (var i = 0; i < row.length; ++i) {
        ...
        if(country != null) {
          ...
          result.push(new generatedQuestion("en", "Geography", "What is the dominant religion (" + religonPercentages[maxIndex] + "%) in " + country[1] + "?", 
      ...
      for (var i = 0; i < row.length; ++i) {
        ...
        if(year != null && winner != null) {
          ...
          if(nominees.length >= 4) {
            var q =new generatedQuestion("en", "Movies", "Who won an Academy Award (Oscar) in " + year[1] + " for his portrayal of " + winner[9] + " in " + winner[6] + "?", 
              winner[3], "1", "2", "3")
